...
2. **Anchor synthesis**: `Trainer._make_anchor_batch` picks a regime (empty / sparse) and `K` **once per batch** via `choose_anchor_count`. For each sample it draws `K` positions along axis 0 via `sample_positions_with_gap` (honoring `min_gap`), samples `B*K` 2D images from the axis-0 pool, and places them into `(sparse, mask)`.
